...
matplotlib & 3.10.9 \\
seaborn & 0.13.2 \\
GPU detectada via CUDA & Não \\
Modelo da GPU & Não aplicável \\
\texttt{torch.cuda.is\_available()} & \texttt{False} \\
Dispositivo utilizado no treinamento & \texttt{cpu} \\
\bottomrule
\end{tabular}
\end{table}
...
A CNN 1D atingiu 0,958880 de acurácia, 0,958585 de F1-score para a classe falsa/sintética, 0,958878 de F1 macro e 0,990750 de AUC-ROC orientada para a classe 0. Entre os baselines, o SVM Linear apresentou desempenho superior ao da Regressão Logística.
...
\midrule
\citeonline{borges2025benchmark} & Fake Reviews PT-BR & Modelos clássicos e contextuais & Benchmark & Métrica não extraída do material local consultado & Referência direta de domínio e dataset \\
\citeonline{kim2014cnn} & Sentenças em inglês & CNN para texto & Acurácia & Não comparável diretamente a este dataset & Referência arquitetural para CNN em texto \\
\citeonline{souza2020bertimbau} & Corpora em português & BERTimbau & Modelo pré-treinado & Não é resultado neste dataset & Referência contextual para português brasileiro e extensão futura \\
Este trabalho & Fake Reviews PT-BR & CNN 1D PyTorch & F1 macro e AUC-ROC & F1 macro 0,958878; AUC 0,990750 & Implementação principal avaliada neste relatório \\
\bottomrule
\end{tabular}
\end{table}
